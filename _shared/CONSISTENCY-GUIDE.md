...
**The drift-check also verifies every shared module block byte-for-byte (2026-09-11).** Each `OTT-*:START … OTT-*:END` block in a dashboard (state migration, profile card/editor, email banner) must equal the trimmed `_shared` source, the email banner must sit inside `#app-shell`, and the session restore must set `APP.email`. **Never hand-edit a dashboard's copy of a shared block.** That is exactly how the four dashboards drifted on 2026-09-11 (email support was patched into each file separately — three different hint texts, one missing banner hook, one dashboard with no banner markup at all). Edit the `_shared` module, re-run its injector on every dashboard, run the check.
...
Every dashboard shows the same **👤 Your Profile** card (role, work email, vertical chips, "✎ Edit Profile & Verticals") directly under the Dashboard hero, with one inline editor: **Your Name / Your Work Email** text inputs (`.pc-input`, real `<form autocomplete="on">` with `name`/`autocomplete` attributes so browser autofill works), then the role and vertical option cards scraped from the welcome screen. Rolled into all 4 dashboards 2026-09-10; each dashboard's previous editor was retired (Fundamentals/Demo 101 `.ev-overlay` modal, PO Match's inline `profileCard`). Name + email fields added 2026-09-11.

**Required fields everywhere (welcome forms, the shared editor, the portal modal): name, a valid work email, role, at least one vertical.**

**The portal has its own modal** (`openProfileEditor()` in `portal.html`, `.pe-*` classes) because it has no welcome screen to scrape and no `APP`. It edits the same `ottimate_profile` with the same fields, rules and copy, and pushes name/email/role/verticals into every dashboard record that belongs to the same person immediately (the dashboards would overlay them on next load anyway; this keeps the portal's own cards current). Keep its option-card styling (`.pe-card`) visually identical to the dashboards' `.select-card`.

### One shared profile (2026-09-10; email 2026-09-11)

A rep enters name / email / role / verticals **once**. `ottimate_profile` = `{ name, email, role, verticals, updatedAt }` is the source of truth; every dashboard's own record keeps a copy (webhooks and emails read from there, and it stays backward compatible).

- **Read:** `ottMigrateState(key)` calls `ottBootstrapProfile(key)` (builds the shared record from existing dashboard records on the first load after deploy), then overlays name/role/verticals/email onto the dashboard record — **only when the names match**, so a record belonging to someone else is never rewritten. **Self-healing both ways:** a record with an email while the profile has none → the profile adopts it; a record saved with a blank email → the profile's email is re-overlaid on the next load.
- **Session restore must copy the email into `APP`** (`APP.email = s.email || ''`) — `inject-statemigrate.js` step 7 adds it and the drift-check enforces it. Without it a reload left `APP.email = ''` and the next `saveState()` wrote the blank back over the record, so the email "disappeared" from every dashboard editor while the portal (which reads the profile) still had it — the 2026-09-11 bug.
- **Write:** the profile editor calls `ottWriteProfile(...)`; first-ever onboarding writes it from `ottSafeSetState` when the welcome form saves (and a later welcome-form / editor save that carries an email fills a profile that had none). Every other dashboard picks the change up on its next load.
- **Renames follow the person.** `ottWriteProfile` rewrites the `name` field (only) of every dashboard record that carried the previous name — otherwise the records would stop matching the profile and the overlay would silently stop applying. Records under any other name are never touched.
- **No record yet + a profile exists** → the record is created from the profile (incl. email) with empty progress, so the dashboard skips its welcome form entirely.
...
**Dashboard view order, identical everywhere:** hero (*"Welcome back, {name}! 👋"* + Role | Verticals + ✏️ Edit link) → 👤 Your Profile → 🎯 Demo Lens → progress card ("Overall Progress" / *n/N modules complete* / `.progress-wrap` bar; exam button when unlocked; **`🏆 View Certificate`** whenever `examPassed`) → module cards in the Demo 101 layout (`.module-card`: icon, ✓ check, "Module N: Title", description, `⏱ time`, status badge, quiz score) → accent-tinted "Reference Materials Always Available" card. Every module carries an estimated duration in its `MODULE_META` (content-measured: ~140 wpm reading + 0.75 min per quiz question + screenshots/steps, rounded up to 5 min) — re-estimate when a module's content changes materially.

**Demo 102 was brought to parity 2026-09-11.** It had kept its own dashboard render (first-name hero without the Edit link, a custom progress card, a different reference card), a `<div>` header with a "Reset" button and an *n/4* counter instead of "Not you?" + a percentage, "Module N:"-prefixed sidebar labels, and — the visible breakage — it lacked the utility classes (`.flex .items-center .justify-between .fw-bold .text-navy .gap-2`) and `.progress-wrap` that the shared profile card and the standard cards depend on, so the profile card's Edit button wrapped under the chips. Its `.mt-*/.mb-*` spacing utilities were also on a different scale and are now the shared one. **Every dashboard needs that utility block** (copy it from Demo 101) before the shared modules render correctly.

**Header badge gotcha:** the badge (`Demo 102`, `Demo 201`) is a `<span>` inside `.app-header-logo`, and `.app-header-logo span` sets the wordmark's accent colour. The badge rule must be `.app-header-logo .app-header-badge` / `.app-header-logo .hdr-badge` (higher specificity) or the badge text takes the wordmark colour and vanishes into its own background (fixed in both 2026-09-11).
...
**Bypass must never require an existing profile** (2026-09-11 regression: Demo 102 was changed to demand name/role/verticals *before* the bypass, so a brand-new user got an alert and no way in). The flow is: confirm → `ottRecordBypass(ownKey, notified)` → re-run init. **Notification timing:** if a profile already exists (`ottBypassIdentity()` — shared profile, else any record with a name) the Enablement email goes out immediately; if not, `bypassNotifyPending:true` is stored on the record and `flushBypassNotice()` sends it once the welcome form is saved (so it carries the real name/role, not "Not yet provided"). The pending flag is carried forward by `ottSafeSetState` and cleared by `ottBypassNotified`, so a reload in between still sends exactly once. Both dashboards' welcome forms show a "Proceeding via bypass" note. **Returning user without a work email (2026-09-11):** after confirming the bypass they get the welcome form pre-filled from the shared profile (name, role, verticals) with only the email required (`showWelcomeEmailOnly`, `window.__ottEmailOnly`); submitting writes the email to the profile and re-runs init so any existing progress restores untouched. Every welcome form has a "← Back / Cancel and return to the Training Portal" link; on the gated dashboards it also withdraws a bypass that was taken moments ago with no profile (`ottCancelPendingBypass`), so the gate returns and no Enablement email is sent.

**Self-attested prerequisites (2026-09-11).** A confirmed bypass is the rep attesting that the prerequisite(s) were completed elsewhere, and the platform reflects that everywhere instead of showing the prerequisites as untouched. The status is **derived, read-only**, by `ottCertStatus(key)` (`state-migration-module.js`; the portal carries an inline copy, `attestation()`) from the bypassing dashboard's own record (`bypassed` + `bypassedAt`, both carried forward by `ottSafeSetState`): Demo 102 bypass → Demo 101 attested; PO Match bypass → Demo 101 + Demo 102 attested. Rules: a real cert (`examPassed`) always wins and is never faked; attested **unlocks the same content a cert does** (downstream gates, portal card locks, Learning Path playbooks + asset chips) but is labelled **"☑ Self-attested"** / **"Completed elsewhere (self-attested · via Demo 102 bypass, Sep 11, 2026)"** with a hover explanation, never "Certified"; it is **not counted as earned** (the progress bar / "n of 3 earned" get a "· k self-attested" suffix; the Certifications Earned strip lists real certs only). The bypassing dashboard's own portal card shows "☑ Opened via bypass on <date>". Reset clears the derived state with the records. Admin approve / reject on top of this: §10.
...
## 9. "Add your work email" banner — single source  → `email-banner-module.js` + `inject-emailbanner.js <file>`

Returning users from before the email field existed have a profile without an email. Every dashboard **and the portal** show the same amber banner — *"📧 Please add your work email to your profile…"* with an **Add email** button — until the profile carries one. Added 2026-09-11.

- **Placement is the whole trick:** the banner markup (`OTT-EMAILBANNER-MARKUP` markers) lives **inside `#app-shell`, between `.app-header` and `.app-body`**, as a flex child. `#app-shell` is `position:fixed; inset:0; z-index:300`, so a banner placed in `<body>` before the screens is completely covered once a session restores — which is why the first attempt "only worked on the portal". The drift-check verifies the placement.
- `getProfileEmail()` reads `ottimate_profile` first, then `APP.email`, then the four records (read-only fallbacks). `updateProfileEmailBanner()` toggles `.on`; it runs at load and from every `showDashboard()/showModule()/showRef()` and after a profile save. `openProfileEmailPrompt()` opens the shared editor on the email field from **any** view (`openProfileEditor({ focusEmail:true })` renders the Dashboard first if the card is not on screen). There is no separate prompt or second write path — one profile, one editor.
- The portal's banner uses the same markup and the same CSS (`#profile-email-banner.on`), but its own `getProfileEmail()/updateProfileEmailBanner()` (it has no `APP`); the button opens the portal modal (§5).
- The banner is hidden in print.

## 10. Bypass verification (admin approve / reject)  → `bypass-verify-module.js` + `inject-bypassverify.js <file>` + `BYPASS-VERIFICATION-SETUP.md`

Built 2026-09-11; the Make.com / Google Sheet side is a one-time setup described in `BYPASS-VERIFICATION-SETUP.md`. Until `OTT_BYPASS_WEBHOOK_URL` / `OTT_BYPASS_FEED_URL` are filled in, behaviour is unchanged (EmailJS bypass email, self-attested only).

- **Identity of a bypass is a random `bypassId`** stored on the bypassing dashboard's record (issued by `ottNotifyBypass` when the event is sent — immediately, or after the welcome form for a brand-new user). The event carries id + name + email + role + dashboard + prereqs + time + site. The verification **feed** (published Sheet CSV or JSON, or `_shared/bypass-verifications.json` as a manual fallback) carries **ids and statuses only** — no rep data is ever published.
- **Every page load** (portal + all dashboards; the module is injected into all five) runs `ottVerifyBypasses()`: fetch the feed, look up this browser's own ids, apply. `approved` → `bypassVerified:true` → **"✓ Admin verified"** everywhere (portal badge, Learning Path, card notes). `rejected` → `bypassed` is **removed** (the gate returns on the next load; the gate shows why and hides the bypass link), `bypassRejected:true`, the portal shows a dismissible banner (`bypassRejectedAck`), and if the rep is inside that dashboard at the time they get a notice with *Back to Portal* — never yanked out mid-module. **Progress in the bypassed dashboard is always kept.** A later `approved` on the same id restores the bypass.
- All bypass fields (`bypassed, bypassedAt, bypassNotifyPending, bypassId, bypassVerified*, bypassRejected*`) are carried forward by `ottSafeSetState`; dashboards never list them in their payloads.
- **Earlier bypasses.** Records with `bypassed:true` but no `bypassedAt` (rolled out 2026-09-10) show the self-attested status with an approximate date taken from the record's creation date ("around September 10, 2026", `ottBypassWhen`), and are registered with Make **once**, flagged `legacy=1` + `since`, on the first load after `OTT_BYPASS_WEBHOOK_URL` is configured (`ottRegisterLegacyBypasses`; never via EmailJS) so they can be approved/rejected too. Pre-2026-09-10 bypasses left no flag at all: the gate asks once more and the confirmation gives them everything new.
- The module is the **same file in the portal and the dashboards** and the drift-check verifies it byte-for-byte in all five. Edit `bypass-verify-module.js` (incl. the two URL constants) → `node _shared/inject-bypassverify.js <each file>` → check.

## 11. Certificates — one design, Certificate IDs, evidence on the portal  → `certificate-module.js` + `cert-id-module.js` + `inject-certificate.js <file>`

Rolled into all 4 dashboards 2026-09-11. Before: Fundamentals / Demo 101 shared one design, PO Match a variant without the verticals line, and Demo 102 a different card that printed **today's date** instead of the certification date, printed the whole page, and whose "Back" returned to the exam results (with a duplicate button).

- **One certificate for every track** (the Fundamentals / Demo 101 design): navy frame, level line, badge, "Certificate of Completion", name, description, **Industry Verticals Trained**, **Final Exam Score**, signature + **Issued: <certDate>** (every dashboard stamps `certDate` when the exam is passed), and a **Certificate ID**. Per-dashboard differences live ONLY in `OTT_CERT_CONFIG` (badge, level, program, description, score total — Demo 102's exam is scored out of 95 points, not questions). The module builds the screen itself (`ottCertEnsureScreen`); the injector retired each dashboard's markup, CSS, `showCertificate()` and `backToDashboardFromCert()`.
- **Identical actions:** 🖨️ Print / Save as PDF (`printCertificate()` — `body.printing-cert` prints the certificate only) · ← Back to Dashboard (always `showDashboard()`).
- **Deep link** `<dashboard>.html#certificate` opens it once the session restores — the portal's "🏆 View My Certificate" on every real cert.
- **Certificate ID** (`cert-id-module.js`, shared with the portal): `OTT-<CODE>-<YYYYMMDD>-<VERTS>-<CHK>`, e.g. `OTT-D101-20260504-SL.HC-4B82` — FNV-1a checksum over code|date|verticals|normalised rep name|salt. It ties the ID to the rep's name and deters casual invention; it is **not** cryptographic proof (the salt is client-side) — Enablement approval (§10) remains the real verification. Certificates issued before 2026-09-11 carry no ID.
...
## 13. Content versions & certification renewal  → `OTT_CONTENT_VERSIONS` (in `state-migration-module.js`) + `renewal-module.js` + `inject-renewal.js <file>`

Added 2026-09-11 (an AE certified PO Match before its quizzes/exam were rewritten). This is the framework for "the content changed after people certified": reconcile what they see, and — when you decide so — make them do it again.

**Bumping a version.** When a dashboard's graded content changes materially, add/update its entry: `ottimate_pomatch_state: { version: '2026-09-11', policy: 'required' | 'optional', note: '…what changed, for the rep…' }`, re-run `inject-statemigrate.js` on the four dashboards **and portal.html** (the portal now carries the state module's helpers too), check, publish. A record whose `certContentV` is older than `version` (or missing — everything certified before versioning existed) is **stale**. Dashboards without an entry are never stale.

- **Stamps.** `ottSafeSetState` stamps `certContentV` (the version the exam was passed under) and `examTotal` (what it was scored against) the moment an exam is passed. Both are carried forward on every save.
- **Display reconciliation (all four dashboards).** A submitted quiz / passed exam whose content is stale shows the **recorded** result ("17/18", against the stored total) with *"Questions updated since your attempt — individual answers can't be shown"* — never re-graded, never painting old answer positions onto new questions. (Before: Fundamentals/Demo 101 re-graded old answers against the new bank; PO Match showed the old score over the new question count.)
- **policy `optional`.** Nothing changes for the rep: certified stays certified; the dashboard shows a dismissible "content updated — retaking recommended" banner, the portal card a note, the Learning Path a hint.
- **policy `required` → renewal.** On the rep's next load of that dashboard (`ottMigrateState` → `ottRenewalDue` → `ottBeginRenewal`): the prior certification is archived to `priorCerts[]` (date, score, total, verticals, version), `renewal: { startedAt, fromV, toV, priorCertDate }` is set, and `examPassed`/scores/modules/quiz answers are reset so the dashboard behaves as a fresh one. Passing the new exam clears `renewal` and stamps the new version; the history stays. **What they see:** portal card *"🔁 Renewal required"* / *Start renewal →* (amber stripe), the Certifications Earned strip shows an amber *"Demo 201 — Renewal Required · Previously certified <date> · for <verticals> · <note>"* card in place of the cert, the dashboard shows a persistent amber banner, the Learning Path says *"Renewal in progress — previously certified <date>"*. Overall progress no longer counts it as earned. **Prerequisite gates still honour the prior certification** while renewal is in progress (`ottGateAllows`), so nothing downstream re-locks.
- **Interaction with the preview gate (§12).** A certified rep is normally exempt from a preview hold; one whose cert predates a *required* version is **not** — they are held with the rest of their role, and the renewal starts on their first visit after release. (Testers in other roles renew immediately.) The portal already shows *"🔁 renewal required"* on their cert card so nothing is a surprise.
- Not built (and where it would go): per-user resets from the admin side → a row in the verification feed (§10) that the client applies like a rejection.
...
- **Renewal notice (dashboards):** `renewal-module.js` + `inject-renewal.js <file>` — in-shell banner for renewal / optional retake; the core (versions, stale detection, archive + reset) is in `state-migration-module.js`, see §13. Run AFTER `inject-previewgate.js`.
